package t.me.p1azmer.velocity.user.hook;
...
public class LuckpermsHook {
    ...
    public static void setGroup(UUID uuid, String string) {
        modifyUser(uuid, user -> {
            String value = user.getCachedData().getMetaData().getSuffix();
            InheritanceNode node = InheritanceNode.builder(string).build();
    ...
    public static TreeMap<String, String> getUserGroups(Player player) {
        return getUserGroups(player != null ? player.getUsername() : null);
    }
    ...
    public static boolean isDonater(Player player) {
        return isDonater(player != null ? player.getUsername() : null);
    }
    ...
    public static Group getGroup(String groupName) {
        return getPlugin().getGroupManager().getGroup(groupName);
    }
    ...
    public static Group getPrimaryGroup(Player player) {
        ...
        try {
            user = getPlugin().getUserManager().loadUser(offlineUUID(player.getUsername())).get();
        } catch (InterruptedException | ExecutionException e) {
            throw new RuntimeException(e);
        }
        if (user == null) {
...
//        return getPlugin().getGroupManager().getGroup(primaryGroup);
    ...
    public static Group getPrimaryGroup(UUID uuid) {
        ...
        Group group = getPlugin().getGroupManager().getGroup("default");
        if (group == null) {
            return getPlugin().getGroupManager().getLoadedGroups().stream().findFirst().orElse(null);
        }
        try {
            user = getPlugin().getUserManager().loadUser(uuid).get();
        } catch (InterruptedException | ExecutionException e) {
